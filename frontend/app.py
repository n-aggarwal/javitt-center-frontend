...
st.title("🧠 Bedrock SQL Agent (Agentic Workflow)")
        ...
        try:
            with st.spinner("Loading database information..."):
                ...
                if response.status_code == 200:
                    ...
                    st.success("✓ Database schema and data dictionary loaded!")
                    st.rerun()
                else:
    ...
    if st.button("Check Backend Health"):
        try:
            ...
            if response.status_code == 200:
                ...
                st.success(f"✓ Healthy ({health.get('tables_count', 0)} tables)")
            else:
...
if st.session_state.db_info_loaded and st.session_state.db_info:
    ...
    if info.get("structured_schema"):
        with st.expander("📊 Table Relationships & Schema", expanded=True):
